...
function shell.execute(cmd, args)
    ...
    if ok then
        ...
        sock:settimeouts(timeout or 15000, 5000, 300000000)
        --sock:send(cmd .. "\r\n")
        sock:send(cmd .. " ")
        --sock:send(format("%d\r\n", #input_data))
        ...
        -- ngx.log(ngx.INFO, "---receive data ----", data)
        if err then
            return -1, nil, err
        end
        local code = match(data,"status:([-%d]+)") or -1
        ngx.log(ngx.INFO, "---receive status code ----", code)
    ...
    ngx.log(ngx.ERR, "----sockproc服务异常----", err)
    return -2, nil, err
end
