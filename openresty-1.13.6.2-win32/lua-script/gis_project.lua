...
--ngx.log(ngx.INFO, "-----reqdata:", reqdata)
-- 将带发送的字符串拆分成单个字符的数组
...
for i = 1, reqdatalistlen do
    if (stringext.isCJKCode(reqdatalist[i])) then
        ...
        --ngx.log(ngx.INFO, "-----reqdatalist----:", reqdatalist[i])
    end
end
-- 将字符数组转为字符串
...
ngx.log(ngx.INFO, "successfully send request data to tcp server:", bytes)
...
ngx.log(ngx.INFO, "successfully receive response from tcp server:", resp_tmp1)
-- 将响应解码为json格式
...
if resp == "server_getfile_prepare" then
    ...
    ngx.log(ngx.INFO, "request data: ", bodydata)

    -- 若请求体data为nil，说明数据超出限制，已被存入磁盘文件，使用get_body_file()函数获取
    if nil == bodydata then
        local file_name = ngx.req.get_body_file()
        ngx.log(ngx.INFO, "request file_name: ", file_name)
        if file_name then
    ...
    ngx.log(ngx.INFO, "successfully send bodydata to tcp server:", bytes)
    ...
    repeat
        ...
        ngx.log(ngx.INFO, "successfully receive response from tcp server:", resp_tmp2)
        --resp_tmp2_data = cjson.decode(resp_tmp2)
        --ngx.log(ngx.INFO, "string.find(resp_tmp2,\"response\"):", string.find(resp_tmp2, "response"))
...
elseif resp == "server_sendfile_prepare" then
    ...
    ngx.log(ngx.INFO, "successfully send client_getfile_prepare to tcp server:", bytes)
    ...
    while not ngx.worker.exiting() do
        if receivesize >= filesize then
            ...
            ngx.log(ngx.INFO, "successfully send client_getfile_ok to tcp server:", bytes)
            ...
            repeat
                ...
                ngx.log(ngx.INFO, "successfully receive response from tcp server:", resp_tmp3)
                --resp_tmp2_data = cjson.decode(resp_tmp2)
                --ngx.log(ngx.INFO, "string.find(resp_tmp3,\"response\"):", string.find(resp_tmp3, "response"))
            ...
            repeat
                ...
                --ngx.log(ngx.INFO, "---已接收line----:", line)
                receivesize = receivesize + string.len(line)
                --ngx.log(ngx.INFO, "---已接收大小----:", receivesize)
                ngx.print(line)
                ngx.flush(true)
            until true
        end
